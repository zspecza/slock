...
export default class BaseAPI extends EventEmitter {
  ...
  constructor(opts) {
    ...
    validateOptions(this.defaults, 'fatal');
  }
   ...
   * @param  {Object} WebSocket=WebSocket used to override socket in mock testing
   * @return {Promise} - a promise for the parsed reply from rtm.start
   */
  async connect(opts, WebSocket=WebSocket) {
    if (opts && opts.dontExecute) return;
...
function cleanOptProps(opts, ...props) {
  props.forEach(prop => delete opts[prop]);
}

function validateOptions(opts, level) {
  ...
  if (invalid_webhook) {
    throw new Error(
      singleLine`
        [${level}] "${opts.webhook}" is not a valid webhook URL.
      `
    );
  }
  if (invalid_token) {
    throw new Error(
      singleLine`
        [${level}] "${opts.token}" is not a valid Slack API token.
      `
    );
  }
  if (invalid_slackbot) {
    throw new Error(
      singleLine`
        [${level}] "${opts.slackbot}" is not a valid slackbot URL.
      `
    );
  }
}
