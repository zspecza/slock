...
export default class BaseAPI extends EventEmitter {
  ...
  constructor(opts) {
    ...
    if (opts == null || !opts) {
      ...
      if (opts.match(VALID_TOKEN)) {
        // set default token if only string was supplied
        this.defaults = { token: opts };
      } else if (VALID_URL.test(opts) && opts.includes('hooks.slack')) {
        // set default webhook
        this.defaults = { webhook: opts };
      } else if (VALID_URL.test(opts) && opts.includes('slackbot?')) {
        this.defaults = { slackbot: opts };
      } else {
        throw new Error(
          singleLine`
            [fatal] the string you provided as Slack options is invalid.
            expected an API token, slackbot URL or incoming webhook URL,
            but instead got "${opts}".
          `
        );
      }
