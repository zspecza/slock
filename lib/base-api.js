...
export default class BaseAPI extends EventEmitter {
  ...
  constructor(opts) {
    ...
    const invalid_token    = this.defaults.token != null &&
                             !VALID_TOKEN.test(this.defaults.token);
    const invalid_webhook  = this.defaults.webhook != null &&
                             !VALID_URL.test(this.defaults.webhook);
    const invalid_slackbot = this.defaults.slackbot != null &&
                             !VALID_URL.test(this.defaults.slackbot);
    if (invalid_webhook) {
      throw new Error(
        singleLine`
